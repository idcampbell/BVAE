...
# Load in the parameters which are relevant to the model construction.
...
class BetaVAE(nn.Module):
    ...
    def loss(self, x_recon, x, mu, logvar):
        ...
        return total_loss/x.shape[0], recon_loss/x.shape[0], KLD/x.shape[0]
    ...
    def weight_init(self):
        for block in self._modules:
            if not isinstance(self._modules[block], (nn.Linear)):
...
class Decoder(nn.Module):
    ...
    def __init__(self):
        ...
        # 1 unit fully connected layer.
        self.single = nn.Linear(1, self.z_dim)
        
        # 10 unit fully connected layer.
        self.fc_z = nn.Linear(self.z_dim, 4096)
    ...
    def restore_partial(self, params):
        ...
        for var_name in params.keys():
            if var_name in net_state_dict:
                if isinstance(net_state_dict[var_name], torch.nn.Parameter):
                    net_state_dict[var_name] = params[var_name].data
