                ...
                for color in colors:
                    ...
                    for size in range(6, 15):
                        ...
                        if shape == "square":
                            ...
                            filename = "square_x" + str(x) + "_y" + str(y) + "_size" + str(size) + "_" + str(color_val) + "_" + color_name + ".png"
                            im.save(datapath + filename, "PNG")
                        if shape == "circle":
                            im = draw_circle(im, [x, y], size*.5, color)
                            filename = "circle_x" + str(x) + "_y" + str(y) + "_size" + str(size) + "_" + str(color_val) + "_" + color_name + ".png"
                            im.save(datapath + filename, "PNG")
...
def generate_dataset(df, cond_list, condname, results_dir):
    ...
    # Now save them.
    train.to_csv(results_dir + condname + '_train.csv')
    test.to_csv(results_dir + condname + '_test.csv')
    held_out.to_csv(results_dir + condname + '_heldout.csv')
...
if __name__ == "__main__":
    ...
    conds = (train_info['color'] == 'green') & (train_info['shape'] == 'square') & (train_info['x'] > 32) & (train_info['y'] > 32)
    cond_name = 'lower_right_green_squares'
    generate_dataset(train_info, conds, cond_name, csvpath)
    
    
    # Now filter out rows that have a red shape in the upper right corner.
    conds = (train_info['color'] == 'red') & (train_info['x'] > 32) & (train_info['y'] < 32)
    cond_name = 'upper_right_red'
    generate_dataset(train_info, conds, cond_name, csvpath)
    
    
    # Now filter out rows that have squares in the lower left corner.
    conds = (train_info['shape'] == 'square') & (train_info['x'] < 32) & (train_info['y'] > 32)
    cond_name = 'lower_right_square'
    generate_dataset(train_info, conds, cond_name, csvpath)

    # Now filter out all green squares.
    conds = (train_info['color'] == 'green') & (train_info['shape'] == 'square')
    cond_name = 'green_squares'
    generate_dataset(train_info, conds, cond_name, csvpath)
